...
- 基于 `httpx.zig` 解析器与 Zyra 自有写出路径的 HTTP/1.1 解析与响应写入
...
```bash
zig build run            # 最小示例（examples/basic.zig）
```

更多专题示例（每个都可单独运行，源码在 `examples/`，含中文注释）：

```bash
zig build run-routing     # 路由：路径参数、路由组、声明式 registerGroupRoutes
zig build run-static      # 静态文件：带 LRU+TTL 路径缓存的 StaticFiles
zig build run-json        # 带类型 JSON API（postJson/getJson）+ 自动 OpenAPI
zig build run-middleware  # 中间件：自定义洋葱、CORS、会话计数
zig build run-logging     # 异步文件日志（AsyncFileSink，经 onReady 启动钩子接入）
```
...
- 启动钩子：`server.onReady(ctx, fn)` 注册一个在 zio 运行时就绪、开始
  ...
  （这些都需要 io，而 `HttpServer` 在内部自管运行时）。`examples/logging.zig`
  演示了用它把异步文件日志接入服务器

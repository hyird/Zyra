...
`.github/workflows/benchmark.yml` 会构建 Zyra、一个 C++ Hical 服务器和
一个 C++ Drogon 服务器，并比较 `wrk` 的吞吐量。基准测试输出
（`benchmarks/*.json`）已被 gitignore。`benchmark/hical/` 和
`benchmark/drogon/` 存放用于对比的 C++ 服务器（CMake/Ninja），不属于
Zig 构建的一部分。
